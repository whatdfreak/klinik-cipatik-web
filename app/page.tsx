...
export default function Home() {
  // ============================================================================
  // LOGIC 1: REAL-TIME WAKTU OPERASIONAL (WIB)
  // ============================================================================
  const [mounted, setMounted] = useState(false);
  const [statusWaktu, setStatusWaktu] = useState({ isOpen: false, text: "Memuat...", color: "emerald" });
  const [bpjsLogoError, setBpjsLogoError] = useState(false);

  useEffect(() => {
    setMounted(true);
    
    // Fungsi agar smooth scrolling berfungsi saat hash link diklik
    ...
    const checkStatusKlinik = () => {
      ...
      if (hari === 0) {
        setStatusWaktu({ isOpen: false, text: "Tutup (Libur)", color: "red" });
      } else {
        const jamMulaiSore = hari === 6 ? 16 : 14; // Sabtu mulai 16:00, Senin-Jumat 14:00
        
        if (jam >= 8 && jam < 12) {
          setStatusWaktu({ isOpen: true, text: "Buka Sekarang", color: "emerald" });
        } else if (jam >= 12 && jam < jamMulaiSore) { // <--- PERBAIKAN DI SINI
          setStatusWaktu({ isOpen: false, text: "Tutup Sementara", color: "amber" });
        } else if (jam >= jamMulaiSore && jam < 18) {
          setStatusWaktu({ isOpen: true, text: "Buka Sekarang", color: "emerald" });
        } else {
          setStatusWaktu({ isOpen: false, text: "Sudah Tutup", color: "red" });
        }
      }
    };
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    const walk = (x - startX) * 2; // Kecepatan scroll (multiplier)
    ...
    <div className="font-sans text-slate-800 bg-slate-50">
      ...
      <section className="relative pt-20 pb-32 md:pt-28 md:pb-36 bg-slate-900 overflow-hidden z-10">
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center w-full">
          ...
          <a 
            href="#jadwal" 
            className="inline-flex items-center gap-2.5 px-4 py-2 rounded-full bg-slate-900/40 border border-slate-700/50 text-slate-100 text-xs sm:text-sm font-medium mb-8 backdrop-blur-md hover:bg-slate-800/60 hover:border-slate-600 transition-all duration-300 group cursor-pointer shadow-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-teal-300 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-900"
          >
            <span className="flex h-2.5 w-2.5 relative">
              {mounted && statusWaktu.isOpen && (
            ...
            <span className={`${mounted ? theme.text : 'text-slate-400'} hidden sm:inline-flex`}>
              {mounted ? statusWaktu.text : "Memuat Jadwal..."}
            </span>
            <span className="hidden md:inline-flex items-center px-2 py-0.5 rounded-full border border-white/15 bg-white/10 text-[10px] font-medium text-white/80 tracking-wide">
              Lihat Jadwal
            </span>
          ...
          <p className="text-base md:text-lg text-teal-50/80 max-w-2xl mx-auto mb-10 leading-relaxed font-light">
      ...
      {/* 2. TRUST BAND (VERSI ENTERPRISE: TENANG, NETRAL, DAN PREMIUM) */}
      <section className="bg-white pt-8 pb-16 border-b border-slate-100 relative z-20">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <p className="text-xs font-semibold text-slate-500 uppercase tracking-[0.18em] mb-10">Fasilitas Kesehatan Tingkat Pertama Melayani</p>
          
          <div className="flex flex-col md:flex-row items-center justify-center gap-10 md:gap-16">
            
            {/* Pasien Umum */}
            <div className="flex items-center gap-4 px-5 py-3 rounded-2xl border border-slate-200 bg-white shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:shadow-md hover:border-slate-300 cursor-default">
              ...
              <div className="flex flex-col text-left">
                <span className="font-semibold text-slate-800">Pasien Umum</span>
                <span className="text-[11px] text-slate-500 uppercase tracking-wide">Layanan Mandiri</span>
              </div>
            </div>
            ...
            {/* BPJS Kesehatan (Highlight dominan namun tetap enterprise) */}
            <div className="flex items-center gap-4 px-6 py-3.5 rounded-2xl border border-slate-300 bg-slate-50/70 shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:shadow-md hover:border-slate-400 cursor-default">
              <div className="h-12 flex items-center justify-center opacity-100">
                {bpjsLogoError ? (
                  <div className="w-12 h-12 rounded-full bg-white border border-slate-300 flex items-center justify-center text-slate-600">
              ...
              <div className="flex flex-col text-left">
                <span className="font-bold text-slate-900">BPJS Kesehatan</span>
                <span className="text-[11px] text-slate-600 uppercase tracking-wide">Mitra Faskes 1</span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4 px-5 py-3 rounded-2xl border border-slate-200 bg-white shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:shadow-md hover:border-slate-300 cursor-default">
              ...
              <div className="flex flex-col text-left">
                <span className="font-semibold text-slate-800">Asuransi Swasta</span>
                <span className="text-[11px] text-slate-500 uppercase tracking-wide">Provider Terpilih</span>
              </div>
            </div>
      ...
      <section id="layanan" className="py-20 md:py-28 bg-slate-50 relative overflow-hidden scroll-mt-20">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="text-center mb-16 max-w-2xl mx-auto">
            <h2 className="text-3xl md:text-4xl font-bold text-slate-900 tracking-tight mb-4">Fasilitas & Layanan Medis</h2>
            <p className="text-slate-600 text-lg leading-relaxed">Infrastruktur kesehatan berstandar klinis dengan dukungan peralatan diagnostik modern, memastikan penanganan yang akurat dan presisi.</p>
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div className="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm hover:shadow-xl hover:shadow-teal-900/5 hover:border-teal-300 transition-all duration-300 transform hover:-translate-y-2 relative overflow-hidden group">
              ...
              <div className="relative z-10">
                ...
                <h3 className="text-xl font-bold text-slate-900 mb-3">Klinik Umum</h3>
                <p className="text-slate-600 text-sm leading-relaxed">Pelayanan diagnostik dan kuratif dasar untuk berbagai keluhan kesehatan keluarga secara komprehensif.</p>
              </div>
            </div>
            ...
            <div className="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm hover:shadow-xl hover:shadow-teal-900/5 hover:border-teal-300 transition-all duration-300 transform hover:-translate-y-2 relative overflow-hidden group">
              ...
              <div className="relative z-10">
                ...
                <h3 className="text-xl font-bold text-slate-900 mb-3">Klinik Gigi</h3>
                <p className="text-slate-600 text-sm leading-relaxed">Perawatan konservasi gigi, tindakan preventif, dan penanganan kegawatdaruratan pada rongga mulut.</p>
              </div>
            </div>
            ...
            <div className="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm hover:shadow-xl hover:shadow-teal-900/5 hover:border-teal-300 transition-all duration-300 transform hover:-translate-y-2 relative overflow-hidden group">
              ...
              <div className="relative z-10">
                ...
                <h3 className="text-xl font-bold text-slate-900 mb-3">Poli KIA & KB</h3>
                <p className="text-slate-600 text-sm leading-relaxed">Pelayanan Kesehatan Ibu dan Anak, mencakup pemantauan Antenatal Care (ANC), imunisasi dasar, dan Keluarga Berencana.</p>
              </div>
            </div>
            ...
            <div className="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm hover:shadow-xl hover:shadow-teal-900/5 hover:border-teal-300 transition-all duration-300 transform hover:-translate-y-2 relative overflow-hidden group">
              ...
              <div className="relative z-10">
                ...
                <h3 className="text-xl font-bold text-slate-900 mb-3">Farmasi & Penunjang</h3>
                <p className="text-slate-600 text-sm leading-relaxed">Penyelenggaraan pelayanan kefarmasian dan pemeriksaan laboratorium sederhana untuk penunjang penegakan diagnosis.</p>
              </div>
            </div>
      ...
      <section id="jadwal" className="py-20 md:py-28 bg-white relative overflow-hidden scroll-mt-20 border-t border-slate-100">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="text-center mb-16 max-w-2xl mx-auto">
            <h2 className="text-3xl md:text-4xl font-bold text-slate-900 tracking-tight mb-4">Jadwal Operasional Pelayanan</h2>
            <p className="text-slate-600 text-lg leading-relaxed">Informasi waktu operasional layanan klinis. Kami berkomitmen memberikan akses kesehatan yang disiplin dan responsif sesuai standar pelayanan.</p>
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8 mb-12">
            {/* Jadwal Umum */}
            <div className="bg-white p-8 rounded-3xl border border-slate-100 shadow-[0_4px_24px_-8px_rgba(20,184,166,0.1)] relative overflow-hidden hover:border-teal-200 transition-colors">
            ...
            <div className="bg-white p-8 rounded-3xl border border-slate-100 shadow-[0_4px_24px_-8px_rgba(20,184,166,0.1)] relative overflow-hidden hover:border-teal-200 transition-colors">
              ...
              <ul className="space-y-4">
                ...
                <li className="flex justify-between items-center text-sm pt-2 border-t border-slate-100">
                  <span className="font-medium text-slate-500">Minggu / Libur</span>
                  <span className="text-slate-500 bg-slate-100 px-3 py-1.5 rounded-md font-semibold text-xs border border-slate-200">Tutup</span>
                </li>
              </ul>
            </div>
          ...
          <div className="bg-teal-50 border border-teal-100 rounded-3xl p-6 md:p-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 shadow-sm mt-8">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <div className="relative mt-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 relative z-10">
              ...
              {/* Step 1: Pendaftaran */}
              <div className="bg-white p-6 md:p-8 rounded-3xl border border-slate-100 shadow-sm hover:shadow-xl hover:shadow-teal-900/5 hover:border-teal-300 transition-all duration-300 transform hover:-translate-y-2 group overflow-hidden relative flex flex-col h-full">
      ...
      {/* 6. TIM MEDIS (DENGAN DRAG TO SCROLL MOUSE LOGIC) */}
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
            <div className="max-w-2xl">
              <h2 className="text-3xl md:text-4xl font-bold tracking-tight mb-4 text-slate-900">Tenaga Medis Profesional</h2>
              <p className="text-slate-600 text-lg leading-relaxed">
                Klinik Pratama Cipatik didukung oleh tenaga kesehatan bersertifikasi kompetensi, berkomitmen penuh pada standar keselamatan dan mutu klinis pasien.
              </p>
            </div>
          ...
          <div 
            ...
            className={`flex overflow-x-auto snap-x snap-mandatory gap-6 pb-8 pt-4 -mx-4 px-4 sm:mx-0 sm:px-0 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden ${isDragging ? 'cursor-grabbing' : 'cursor-grab'}`}
          ...
          >
            {dataTimMedis.map((medis) => (
              <div key={medis.id} className="relative min-w-[280px] md:min-w-[320px] aspect-[3/4] snap-center rounded-3xl overflow-hidden group shadow-lg hover:shadow-xl border border-slate-200 transition-all duration-300 flex-shrink-0">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-slate-900 via-slate-900/50 to-transparent opacity-80 group-hover:opacity-90 transition-opacity pointer-events-none"></div>
                
                <div className="absolute bottom-0 left-0 w-full p-6 transition-transform duration-500 md:translate-y-12 md:group-hover:translate-y-0 pointer-events-none">
